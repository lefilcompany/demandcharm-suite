...
Deno.test("calendar helpers calculate tomorrow in America/Recife", () => {
  const now = new Date("2026-07-14T20:00:00.000Z"); // 17:00 in Recife
...
Deno.test("delivery channels respect global and deadline preferences", () => {
  ...
  assertEquals(
    getEnabledDeliveryChannels({ deadlineReminders: false }),
    [],
  );
});
