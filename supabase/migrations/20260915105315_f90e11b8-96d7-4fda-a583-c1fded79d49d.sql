...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    ...
    v_type := 'warning';
  ELSIF TG_OP = 'UPDATE' AND OLD.is_primary IS DISTINCT FROM NEW.is_primary AND NEW.is_primary THEN
...
REVOKE ALL ON FUNCTION public.notify_demand_assignee_membership_change() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.notify_demand_assignee_membership_change() FROM anon;
REVOKE ALL ON FUNCTION public.notify_demand_assignee_membership_change() FROM authenticated;
...
AFTER UPDATE OF is_primary OR DELETE ON public.demand_assignees
