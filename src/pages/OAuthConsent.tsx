...
export default function OAuthConsent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const api = oauthApi();
      if (!api) { setError("OAuth API indisponível. Faça um hard-reload (Ctrl/Cmd+Shift+R)."); return; }
  ...
  async function decide(approve: boolean) {
    setBusy(true);
    const api = oauthApi();
    if (!api) { setBusy(false); setError("OAuth API indisponível."); return; }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-6 bg-background">
      <Card className="max-w-md w-full">
        <CardHeader><CardTitle>Conectar {clientName} ao SoMA</CardTitle></CardHeader>
