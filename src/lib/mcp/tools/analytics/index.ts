...
/**
 * Board-scoped metrics come from the canonical SQL module (`get_board_metrics`),
 * shared with the in-app board assistant. Team-scoped tools below still aggregate
 * in the handler because the SQL module is per board.
 */

const zIsoDate = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "use YYYY-MM-DD");

export const boardSummaryStatsTool = defineTool({
  name: "board_summary_stats",
  title: "Board summary stats",
  description:
    "Canonical board metrics: current state (open, delivered, overdue, due soon, by stage/priority/service/responsible), " +
    "period flow (created, delivered on time vs late, on-time rate, requests by status) and the monthly creation limit. " +
    "Same numbers the in-app assistant and dashboards use. Omit from/to for the full history.",
  inputSchema: {
    board_id: zUuid,
  ...
  handler: async ({ board_id, from, to, member_id, timezone }, ctx) => {
    ...
    if (!data || typeof data !== "object") {
      return { content: [{ type: "text", text: "NOT_FOUND: board not found or not accessible" }], isError: true } as any;
    }
    return ok(data as Record<string, unknown>, { open_url: urls.board(board_id) });
  },
});
