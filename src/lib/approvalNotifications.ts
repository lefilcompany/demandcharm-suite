...
export async function notifyApproval({
  ...
  if (filtered.inapp.length > 0) {
    try {
      const { error: rpcError } = await supabase.rpc("create_approval_notifications", {
        ...
        p_recipient_ids: filtered.inapp,
        p_title: title,
        p_message: message,
        p_link: link,
        p_type: "info",
      });
      if (rpcError) console.error("Error inserting approval notifications (RPC):", rpcError);
    } catch (error) {
  ...
  if (filtered.email.length > 0) {
    try {
      ...
      await Promise.all(
        filtered.email.map(async (userId) => {
          try {
