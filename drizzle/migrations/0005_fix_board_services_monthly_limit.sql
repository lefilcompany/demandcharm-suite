...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM jsonb_array_elements(v_stages) s WHERE lower(trim(s.value->>'name')) = 'solicitações'
  ) THEN
    v_stages := '[{"name":"Solicitações","color":"#EC4899","adjustment_type":"none"}]'::jsonb || v_stages;
  END IF;
